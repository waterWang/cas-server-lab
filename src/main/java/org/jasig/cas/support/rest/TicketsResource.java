 ...
 *
 * This class implements main CAS RESTful resource for vending/deleting TGTs and
 * vending STs:
 *
 * <ul>
...
public class TicketsResource {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(TicketsResource.class);
	...
	public final ResponseEntity<String> createTicketGrantingTicket(
		...
		try (Formatter fmt = new Formatter()) {
			...
			fmt.format("%s %s", HttpStatus.CREATED,
			...
			for (final Map.Entry<String, Class<? extends Exception>> handlerErrorEntry : e
					.getHandlerErrors().entrySet()) {
				authnExceptions.add(handlerErrorEntry.getValue()
						.getSimpleName());
			}
			...
			return new ResponseEntity<>(this.jacksonObjectMapper.writer()
					.withDefaultPrettyPrinter().writeValueAsString(errorsMap),
	 ...
	 */
	@SuppressWarnings("rawtypes")
	@RequestMapping(value = "/accounts/pwd/{username}", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<?> getPwd(@PathVariable("username") String username)
			throws URISyntaxException {
		LOGGER.debug("REST request to get pwd");

		String dbPassword = userInfoService.getPWD(username);
		if (null == dbPassword) {
	 ...
	 *
	 * @param requestBody
	 *            service application/x-www-form-urlencoded value
	 * @param tgtId
	 *            ticket granting ticket id URI path param
	...
	public final ResponseEntity<String> createServiceTicket(
		...
		try {
			final String serviceId = requestBody
					.getFirst(CasProtocolConstants.PARAMETER_SERVICE);
			final AuthenticationContextBuilder builder = new DefaultAuthenticationContextBuilder(
					this.authenticationSystemSupport
							.getPrincipalElectionStrategy());

			final Service service = this.webApplicationServiceFactory
					.createService(serviceId);
			final AuthenticationContext authenticationContext = builder
					.collect(
							this.ticketRegistrySupport
									.getAuthenticationFrom(tgtId)).build(
							service);

			final ServiceTicket serviceTicketId = this.centralAuthenticationService
					.grantServiceTicket(tgtId, service, authenticationContext);
	...
	public final ResponseEntity<String> deleteTicketGrantingTicket(
			@PathVariable("tgtId") final String tgtId) {
	...
	public void setTicketRegistrySupport(
	...
	private static class DefaultCredentialFactory implements CredentialFactory {
		@Override
		public Credential fromRequestBody(
				@NotNull final MultiValueMap<String, String> requestBody) {
